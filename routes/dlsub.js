...
router.get("/subtitles/:path/:lan/:id", async (req, res) => {
  ...
  try {
    if (!req.params.path.length || !req.params.lan.length || !req.params.id.length) {
    ...
    let type = $("#details ul li:nth-child(7)").text().replace("Release type:", "").trim() || null;
    let files = $("#details ul li:nth-child(5)").text().replace("Files:", "").trim() || null;
    let downloads = $("#details ul li:last-child").text().replace("Downloads:", "").trim() || null;
    let uploadedAt = $("#details ul li:first-child").text().replace("Online:", "").trim() || null;
    ...
    results = { title, author, type, files, downloads, uploadedAt, download, releaseInfo };
